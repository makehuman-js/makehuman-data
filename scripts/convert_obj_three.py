...
Source: https://github.com/timoxley/threejs/blob/master/utils/exporters/obj/convert_obj_three.py
Modified by wassname to export skeleton, skinWeights, and to convert face groups into material indexes
...
def parse_mtl(fname):
    ...
    Ref:
    - https://github.com/mrdoob/three.js/blob/master/examples/js/loaders/MTLLoader.js#L389
    - https://github.com/mrdoob/three.js/blob/master/src/loaders/MaterialLoader.js#L54
    """
    ...
    for line in fileinput.input(fname):
        ...
        if len(chunks) > 0:
            ...
            if (chunks[0] == "bump" or chunks[0] == "bump") and len(chunks) == 2:
                materials[identifier]["mapBump"] = texture_relative_path(chunks[1])

            # ambient occlusion texture
            if chunks[0] == "map_ao" and len(chunks) == 2:
                materials[identifier]["mapAO"] = texture_relative_path(chunks[1])

            # normal texture map
            if chunks[0] == "map_norm" and len(chunks) == 2:
                materials[identifier]["mapNormal"] = texture_relative_path(chunks[1])

            # specular highlight map, not supported in three.loader
            # if chunks[0] == "map_Ns" and len(chunks) == 2:
            #     materials[identifier]["mapNormal"] = texture_relative_path(chunks[1])

            # not supported in threejs loader.js yet
            # if chunks[0] == "disp" and len(chunks) == 2:
            #     materials[identifier]["mapDisplacement"] = texture_relative_path(chunks[1])
            ...
            # Ambient color no longer supported
            ...
            if chunks[0] == "wireframe" and len(chunks) == 2:
                materials[identifier]["wireframe"] = int(chunks[1])
            ...
            if "colorSpecular" in materials[identifier]:
                materials[identifier]["shading"] = "phong"
            elif not "colorEmissive" in materials[identifier]:
                materials[identifier]["shading"] = "basic"
...
def face_groups_to_materials(faces):
    """Convert face groups to materials with material indices."""
...
# API - ASCII converter
# #####################################################
def convert_ascii(infile, morphfiles='', colorfiles='', outfile='converted_obj.json', licence='', mtllib=''):
    ...
    Params:
        infile - a string giving the location of the .obj file
        outfile - string giving location of the output json
        morphfiles - string containing a glob giving location of morphfile .obj files e.g .morphfiles/*.obj
        colorfiles - string containing a glob giving location of colorfiles
        licence - string to add licence metadata to the output file
        mtllib - manually include a mtl file
    """
    ...
    # parse OBJ / MTL files
    faces, vertices, uvs, normals, materials, inline_mtllib = parse_obj(infile)

    # make new materials from facegroups so we hide them etc
    face_group_materials = face_groups_to_materials(faces)
    materials.update(face_group_materials)

    if not mtllib:
        mtllib = inline_mtllib
    else:
        mtlname = os.path.splitext(os.path.basename(mtllib))[0]
        if 0 in materials:
            del materials[0]
        materials[mtlname] = 0
    ...
    print "%d vertices, %d faces, %d materials, %d morph targets, %d bones, %d skinWeights" % (len(vertices), len(faces), len(materials), len(morphTargets), len(skinWeights), len(bones))
